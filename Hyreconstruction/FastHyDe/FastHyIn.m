function [image_fasthyde, time] = FastHyIn(img_ori, M,  noise_type, iid, k_subspace)
% Input: -------------1
%
...
[Lines, Columns, B] = size(img_ori);%[512,217,204]
N=Lines*Columns;%512*217

%   vec = @(x) x(:);
M_2d = reshape(M, N, B)';%(512,512*217,204)
M_pixel = ones(Lines, Columns);
M_pixel(find(sum(M,3)<B))=0;
loc_complete = find(M_pixel(:)==1);145
...
for i_corrupt = find(M_pixel==0)'
    ...
    y_i = Y_ori(:,i_corrupt);
    y_i(strp_band)=[];
    eigen_Y(:,i_corrupt) = pinv(E'*M_i'*M_i*E)*E'*M_i'*y_i;
end
